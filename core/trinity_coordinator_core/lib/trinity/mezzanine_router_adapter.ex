defmodule Trinity.MezzanineRouterAdapter do
  ...
  def route(route_request, opts) when is_map(route_request) and is_list(opts) do
    with :ok <- reject_raw(route_request),
         ...
         {:ok, model_profiles} <- model_profiles(route_request),
         {:ok, config} <- config(route_request, model_profiles, opts),
         {:ok, decision} <-
  ...
  defp model_profiles(route_request) do
  ...
  defp config(route_request, model_profiles, opts) do
    config_attrs =
      opts
      |> Keyword.get(:trinity_config, default_config(route_request, model_profiles, opts))
  ...
  defp default_config(route_request, model_profiles, opts) do
    role_ref = Keyword.get(opts, :default_role_ref, "role://trinity/default")
    selected_model_profile_ref = List.first(model_profiles)
