defmodule Trinity.Bridge.Trace.JsonlSinkTest do
  ...
  test "writes coordinator-compatible route JSONL bytes" do
    path = tmp_path("route")
  ...
  test "redacts sensitive fields and materialized secret values" do
  ...
  test "context write is inert when disabled and writes with context run id when enabled" do
  ...
  test "can include stable AITrace metadata on demand" do
    path = tmp_path("aitrace")
  ...
  defp tmp_path(label) do
    Path.join(
      System.tmp_dir!(),
      "trinity-bridge-trace-#{label}-#{System.unique_integer([:positive])}.jsonl"
    )
  end
end
